...
function App() {
    ...
    const adicionarTarefa = (): void => {
        if (novoTodo !== "") {
            const newId = crypto.randomUUID(); // <= Para gerar um numero(id) aleatorio
    ...
    return (
        <div className={`app ${theme}`}>
            <div className={`container ${theme}`}>
                <h1>
                    Lista de Tarefa - {obterTarefasCompletas().length} /{" "}
                    {todos.length}
                </h1>
                ...
                <button onClick={toggleTheme}>Alterar Tema { theme === 'light' ? 'Escuro' : 'Claro' }</button>
            </div>
        </div>
    );
}
